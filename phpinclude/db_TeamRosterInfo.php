<?php
/*
 *  filename:       db_TeamRosterInfo.php
 */
...
if ($methodType === "GET") {
	...
	if (isset($_GET['getTeammates'], $_SESSION['username'], $_SESSION['pw'], $_SESSION['usergroup']) && !empty($_GET['getTeammates']) && !empty($_SESSION['username']) && !empty($_SESSION['pw']) && !empty($_SESSION['usergroup'])) {
		...
		if ($_SESSION['usergroup'] == 1 || $_SESSION['usergroup'] == 10 || $_SESSION['usergroup'] == 99) {
		    try {
			$list = $conn->prepare('SELECT null AS control, S_FirstName, S_LastName, r.StudentNo, Stu_is_Manager, FormBCSS, FormAC, FeePaid, FeeAmountPaid, UniformNo, UniformReturned, Point_Team, Point_Bonus, null AS edit FROM student s INNER JOIN roster r ON s.StudentNo = r.StudentNo INNER JOIN team t ON r.Team_ID = t.Team_ID WHERE r.Team_ID = :TeamID ORDER BY S_FirstName, S_LastName ASC');
